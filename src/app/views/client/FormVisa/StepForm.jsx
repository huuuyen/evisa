...
import FormEvisa from 'app/components/ForrmAddVisa/Index';
import FormPayment from 'app/components/ForrmAddVisa/PayEvisaFee';


function getSteps() {
    return ["Fulfill foreigner’s information", "Review application form", "Pay e-visa fees"];
}
    ...
    return (
        <Box >
            <Box style={{ textAlign: 'center', marginBottom: '20px' }}>
                <Typography style={{ fontSize: "30px", fontWeight: "600", }} >For outside Vietnam Foreigners personally applying for E-visa</Typography>
            </Box>
            ...
            <Box mt={4}>
                {activeStep === steps.length ? (
                    ...
                    <Box>
                        {activeStep !== 2 ? <FormEvisa /> : <FormPayment />}
                        <Box container spacing={1} style={StyleButtonSubmit}>
                            <StyledButton onClick={handleNext} variant="contained" style={{ background: "#1EB15A" }}>
                                {activeStep === 0 ? ' Review application form' : activeStep === 1 ? 'Payment' : 'Pay e-visa fee'}
                            </StyledButton>
